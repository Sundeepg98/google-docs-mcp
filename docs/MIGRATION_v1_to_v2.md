...
- **Per-user Apps Script HMAC (WIRED in v2.0c).** The `apps_script_hmac_key` column landed in v2.0a; v2.0c wires it end-to-end: `setup_apps_script` provisions one per user and bakes it into the deployed `restructure.gs`, `_call_webapp` signs every `/exec` POST (`mcp_ts` + `mcp_sig` query params; Apps Script delivers no HTTP request headers to `doPost`, so the query string is the only viable transport), and `restructure.gs::doPost` verifies (fail-closed) before acting. THREAT_MODEL §4 row 5 is now CLOSED: `/exec` is authenticated, not URL-secrecy-only.
...
### 2.2 `apps_script_hmac_key` is required for all users (verify-path wired in v2.0c)

The v2.0a migration added the `apps_script_hmac_key` column to `user_state`; v2.0c consumes it. `setup_apps_script` provisions a fresh 64-char hex key per user (read-or-create, so re-runs are idempotent) and bakes it into that user's deployed `restructure.gs`. Existing users (rows that pre-date v2.0a) have `NULL` in this column — the migration script (`scripts/migrate_existing_users.py`) backfills them, AND any user who re-runs install gets a key minted on the spot.

With v2.0c live, an existing user whose deployed script still pre-dates the HMAC bake will see their next retrofit fail with `stage:'auth'` until they re-run `gdocs_install_automation` (which re-deploys the script carrying their key). Re-running install is the single remediation; it is not data-destructive.
...
The dry-run prints which `user_state` rows would receive a backfilled `apps_script_hmac_key` (now consumed at runtime by the v2.0c verify-path — see §2.2) and exits without writing. Capture the output — the row count tells you how many users need the backfill.
...
The script provisions one 64-char hex `apps_script_hmac_key` per legacy user (consumed at runtime by the v2.0c verify-path), writes back to `user_state.db`, and prints `migrated: <count> users`. Verify the row count matches the dry-run from Step 2. (Users re-running `gdocs_install_automation` also get a key minted + baked into their re-deployed script.)
