...
- **Per-user Apps Script HMAC (WIRED in v2.0c).** The `apps_script_hmac_key` column landed in v2.0a; v2.0c wires it end-to-end: `setup_apps_script` provisions one per user and bakes it into the deployed `restructure.gs`, `_call_webapp` signs every `/exec` POST (`X-MCP-Signature` + `X-MCP-Timestamp`), and `restructure.gs::doPost` verifies (fail-closed) before acting. THREAT_MODEL §4 row 5 is now CLOSED — `/exec` is authenticated, not URL-secrecy-only.
