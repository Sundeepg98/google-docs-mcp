...
| 8 | **`drive.readonly` scope stays in baseline** | Required for the cloud-chat `gdocs_tab_existing_doc(drive_file_id=…)` ingestion path; users routinely pass a Drive ID of a doc THIS app didn't create (CASA testing-mode bypass applies; not currently a posture concern) | Read-only — no mutation possible. Scope's effect is read-only access to Docs/Sheets/Slides files the user explicitly identifies by ID | Not planned (correct trade-off for the cloud-chat UX) |
