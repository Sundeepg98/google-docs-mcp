...
> - **Tool surface is now 134 tools across 12 services** (the frozen body says 41->57). Calendar/Tasks/Forms/Contacts/Gmail services + the GAS automation layer + REST-enrichment passes all shipped after the freeze (PR refs in `ROADMAP.md`). The **code requests 15 workspace / 17 connector OAuth scopes** now (13 SENSITIVE + 4 non-sensitive, zero restricted, no CASA). Full deploy-staging detail: **`SCOPE_EXPANSION_PLAN.md`**.
> - **Demo status:** the original demo `hBuuDemD8Js` was **REJECTED** (chat-only / insufficient); `r7ZB1YeT3SE` was a later interim recording (8-then-13-scope era). BOTH are SUPERSEDED by a **PENDING 17-scope re-record** (not yet recorded); do not present either as current.
> - **Serving-URL cutover (2026-06-14):** the live serving URL is now the custom domain **`https://mcp.appscriptly.com/mcp`** (TLS cert + Cloudflare DNS + redirect URIs + the consent screen (raised to 17 scopes at go-live) + `TRUSTED_HOSTS` secret), still backed by the same Fly app `sundeepg98-docs-mcp`. The apex `appscriptly.com` remains the Cloudflare Pages landing/branding site only.
