...
- **Boot guards (fail loud before serving):** (1) any discovery import error → `RuntimeError` at module load; (2) `on_duplicate="error"` turns a duplicate tool name into a boot crash; (3) `_MIN_EXPECTED_TOOL_COUNT` floor (a moving floor that tracks the shipped surface — currently 134) catches a silent surface drop.
