...
from google_docs_mcp.tool_schemas import AS_INSTALL_SHEET_DASHBOARD_OUTPUT_SCHEMA
...
def as_install_sheet_dashboard(
    ...
    return {
        ...
        "activation_instructions": (
            ...
            f"and run the `installTrigger` function once (Run button), "
            f"then approve the authorization prompt. That activates the "
            f"{schedule} schedule for `{handler}`; it then runs on "
            f"Google's clock with no further action."
        ),
    }
