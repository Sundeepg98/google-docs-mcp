...
def gdrive_get_signed_upload_url(
    ...
    **v2.1 — bound to your user identity.** The URL embeds the calling
    ...
    minted["usage_hint"] = (
        ...
        "dry_run ('1': returns the conversion PLAN only (split_strategy_used, heading1_found, "
        "planned tabs with nesting, placeholder decision, fidelity warnings) and creates "
        "NOTHING (no doc, no Drive change) and does NOT consume this URL, so the SAME URL "
        "then performs the real convert. Always synchronous; one input only; wins over async), "
        "drive_file_id (convert an existing Drive docx/Doc that is APP-ACCESSIBLE, "
        "meaning created by this app or explicitly shared with it under the drive.file "
        "scope: pass data only, no files=; an explicit title is honored verbatim on "
        "every entry point. CAP: a file this app has never touched cannot be read and "
        "returns an insufficient-scope error; to convert one, upload its .docx bytes to "
        "this same URL via files= instead, which needs no Drive read scope), "
