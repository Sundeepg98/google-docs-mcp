...
## [Unreleased] — PR-α reframe + PR-Δ1 spec compliance + scope union + PR-Δ2 security posture artifacts + PR-Δ3 hardening + retry adapter + PR-Δ3.5 retry adoption + PR-Δ4 DR + observability + PR-Δ5 commercial-ready engineering + PR-Δ5.5 rename to appscriptly + PR-Δ3-hotfix Dockerfile home-dir fix + PR-Δ6 Vercel pilot + PR-Δ7 bound-script generator + PR-Δ8 doc-menu installer + PR-Δ10 custom-function installer + PR-Δ9 sheet-dashboard installer + PR-Δ11 slides-to-video render half + PR-Δ12 slides-to-video encode half + auto-discovery registration refactor + OAuth-state persistence fix (FASTMCP_HOME on volume) + encode_video resource bounds + SQLITE_READONLY volume-ownership fix (runtime chown + setpriv) + base-tier drive.readonly drop (free-publish scope cleanup) + pure-REST tab transplant (Apps Script /exec step removed from the tabs pipeline)

### Changed

- **Web-app runtime heal + install honesty: consent-preserving redeploy, needs_activation as data, and the true (empty) gating set** (`src/appscriptly/setup_apps_script.py`, `src/appscriptly/services/gas_deploy/tools.py`, `src/appscriptly/services/admin/tools.py`, `src/appscriptly/tool_schemas.py`). Fixes the three runtime-heal findings from the 2026-07-10 live activation session (PR-D; spec `_audit/2026-07-10-pr-d-brief.md`).
  - **`probe_webapp_health` is now four-way** - the old two-way live/DEAD verdict lumped two OPPOSITE conditions: `HEALTHY` (200 + JSON), `CONSENT_GATED` (403 door page or a 200 HTML interstitial - the deployment EXISTS behind Google's one-time per-script consent), `GONE` (404/definitive 4xx - the deployment no longer exists), `UNKNOWN` (transport trouble / 5xx / 429).
  - **Heal preserves consent (Finding A):** a GONE cached deployment now cuts a fresh version + web-app deployment on the SAME script project (the one-time Run + Allow the user already granted survives); a NEW project is minted only when the project itself is gone (`projects.get` 404). The old heal cleared everything and re-provisioned a new project on every "dead" reading - including consent-gated ones, where the new project's consent gate starts over: the #221 thrash. The HMAC key still survives the reset in both ledger backends.
  - **Installer returns `needs_activation` as DATA (Finding B):** after provisioning, one probe classifies the fresh deployment; the consent door returns `{status: "needs_activation", activation_url, message}` (editor URL + the one-time Run + Allow instructions) instead of the pre-PR-D behaviors (claiming "ready" for a gated endpoint, or the heal path's misdiagnosed "re-authorize / API disabled" RuntimeError). A just-created deployment that 404s raises a retryable ToolError naming the same-project retry guarantee. `gdocs_install_automation`/`gdocs_setup_apps_script` output schema gained the `needs_activation` variant (tool NAMES unchanged - golden surface untouched).
  - **`server_health` splits the door from the grave (Finding C):** CONSENT_GATED reads as `exec: "needs_activation"` with the script-editor remediation URL; a 404-dead deployment now reads as `installed: false` + `exec: "not_installed"` with a redeploy detail (the old code sent consent instructions that cannot fix a deleted deployment - the mislabel that started the investigation). The `gates` field now states the code-derived truth: NOTHING at runtime consumes this web app since the pure-REST transplant (#222); as_* installers depend on the Apps Script API toggle + OAuth scopes, not on this runtime - and frames the web app per the operator decision (2026-07-10) as dormant infrastructure kept for future features.
  - **Riders:** `effective_convert_title` (services/drive/api.py) is now the single title rule for all three convert naming sites AND docx_import's on_conflict predictor (the #235 reviewer's ask); on_conflict docs state that comparison is against pre-existing docs only, never batch siblings; README documents claude.ai's client-side "No approval received" popup and corrects the stale claim that the web app is a prerequisite for `gdocs_tab_existing_doc`.

- **Convert pipeline hardened for data safety: pinned step order, completion manifest in every response, keep-not-trash on partial failure, and an on_conflict policy** (`src/appscriptly/docx_import.py`, `src/appscriptly/retrofit.py`, `src/appscriptly/services/docs/tools.py`, `src/appscriptly/http_server/routes/convert.py`, `src/appscriptly/tool_schemas.py`). Response to the 2026-07-09 field session's S2.5 DATA-LOSS incident (an OOM-killed convert left 4 sections existing ONLY inside "Tab 1"; the documented cleanup deleted that tab and destroyed them) and its T2.x feedback.
  - **Pipeline order is now a pinned contract (amended 2026-07-10):** import (FINAL title at Drive-create time) -> tab shells -> transplant ALL -> verify ALL -> carve the moved ranges out of the placeholder -> cosmetics (icons) -> placeholder policy LAST, with every cosmetic failure downgraded to a response warning (never fatal, never before a DATA-safety step, and never blocking the placeholder step that follows). Cosmetics sit between carve and placeholder because the tools stream live-proved a Google-side defect: once a document's ORIGINAL FIRST TAB is deleted, every later `updateDocumentTabProperties` (icon sets AND tab renames) returns 500 permanently - so icons must land while that tab still exists. A successful placeholder delete therefore appends an advisory warning that tab icons/renames are no longer editable on the produced doc (use `placeholder_behavior="rename"`/`"keep"` to avoid).
  - **Completion manifest in EVERY convert response:** `completion: {steps_completed, moved_sections, pending_sections}`. A step absent from `steps_completed` failed or still has work; a section in `pending_sections` exists ONLY inside the placeholder tab. Consumers (and the gdocs_delete_tab force-guard) can now refuse unsafe placeholder deletion mechanically. EXECUTED and VERIFIED are kept distinct (review finding on the #226 interim text manifest, which called executed tabs "fully transplanted" even when the failure was a verify shortfall): "transplant" is claimed only when every plan finished executing, `moved_sections` only lists tabs that PASSED a fresh verify - so "transplant present, verify absent" reads unambiguously as a verify shortfall and "transplant absent" as an execution death.
  - **Partial failure keeps the doc (S2.5):** once content has started moving, a transplant/verify failure NO LONGER rolls back shells + trashes the working copy. The doc is kept and the response (HTTP 500 on the REST path; a structured return on the tool path) carries `error` + the manifest, with moved/pending classified by a fresh per-tab verify (classification-fetch failure downgrades to all-pending: the manifest under-promises, never over-promises). A failure BEFORE any content write still removes the shells and trashes the staging copy - and the error now NAMES the trashed doc_id and says it is recoverable from Drive trash for 30 days (review finding: on the HTTP upload path the staging copy can be the only Drive-side copy of an ephemeral upload, so "the source document is untouched" alone was misleading).
  - **`delete` placeholder policy now carves first and refuses to destroy sole-copy content:** the moved ranges are carved out for every policy, so a failed tab removal strands an EMPTY tab (cosmetic) instead of a full copy; and when content that belongs to NO section (e.g. text before the first heading) remains, `delete` downgrades to keep-with-warning instead of destroying the only copy.
  - **T2.1 response fields on both tool + REST paths:** `heading1_found`, `tabs_created`, `placeholder` ("deleted"|"renamed"|"kept"|"none" - the ACTUAL outcome), `warnings[]`, `on_conflict_action`. `action` semantics tightened: "replaced" is reported only when a prior version was actually trashed (it previously claimed "replaced" even when the trash failed or the no-splits early return skipped it).
  - **T2.2 placeholder determinism:** `placeholder_behavior` documented on `POST /api/convert` (it was parsed but undocumented) and the default pinned to `delete` on BOTH paths by regression test (pipeline, retrofit, tool signature, and route form default). The operator-observed "stray empty Tab 1 on one doc in four" traced to the /exec-era pipeline (the GAS restructure carved, then a separate Python delete could fail or die in between) plus a mid-session deploy changing defaults; outcomes are now a pure function of inputs and always REPORTED via the `placeholder` field + manifest.
  - **T2.3 `on_conflict` param (tool + REST):** `new` (default) | `replace` (after a fully successful build, trash the newest prior same-title app-visible Google Doc; response says `replaced` + `replaced_doc_id`) | `skip` (return the existing doc's info, `action="skipped"`, converting nothing). Title lookup runs under `drive.file` visibility ONLY (docs this app created/was granted); it filters out .docx sources and the just-created doc, and an explicit `replace_doc_id` wins over the lookup. Retries after a timeout no longer litter Drive with duplicate docs.
  - **BUG 2a (tmp-name docs):** `/api/convert` derives an absent `title` from the UPLOADED filename (never the server temp-file stem), and the retrofit path resolves its title from the ORIGINAL source before converting through its temp copy - a convert that dies mid-way now always leaves a doc findable by its real title.
  - Golden tool surface: zero diff (no tools added/removed; `gdocs_tab_existing_doc` gains the `on_conflict` param and the output schema gains the T2.1/manifest fields, both within the existing tool).

- **Tabs conversion rewired to a pure Docs-REST content transplant; the per-user Apps Script /exec web-app step is gone from the pipeline** (`src/appscriptly/docx_import.py`, new `src/appscriptly/services/docs/content_transplant.py`). Root cause of the replacement: an API-deployed per-user Apps Script that requires a sensitive scope has NO per-script consent grant (the REST deploy path never collects one, and Google provides no headless way to), so Google 403s every anonymous /exec POST at its consent door BEFORE doPost runs. The lossless-convert feature was therefore dead for every cloud user for the entire API-deploy era, and each attempt orphaned a shell-riddled Drive copy. The new step 5 re-emits each split range into its tab shell via `documents.batchUpdate` (read shape passes through to `updateTextStyle` / `updateParagraphStyle` / `updateTableCellStyle` / etc. with computed field masks; tables use the proven create/re-fetch/reverse-fill sync-point pattern; every Location/Range carries an explicit tabId; UTF-16 code-unit offsets per the R6/#184 contract). Runs entirely under the already-granted `documents` scope: no script project, no deployment, no second consent surface, no activation click. TRANSACTIONAL: all new tabs are built and VERIFIED first; the source content leaves the primary tab last (tab delete, or range carve for `placeholder_behavior=rename/keep`); a mid-transplant failure deletes the new shells and trashes the working copy, so a half-converted doc can never survive. Fidelity is high but not lossless: non-representable elements (equations, embedded drawings, positioned objects, footnotes, auto-text, column breaks, date chips) are DETECTED at plan time and surfaced as per-doc warnings, never dropped silently; degradations (horizontal rule as bottom border, list-numbering restarts, static chart snapshots, TOC placeholder) surface as info notes. The detection registry (`DROPPED_KINDS` / `DEGRADED_KINDS`) is exported for the follow-up channel that closes the tail. Deleted with the POST: `_call_webapp`, `_resolve_webapp_url`, `_resolve_webapp_hmac_key`, `_body_is_json`, `_splits_to_json` and the webapp-URL install gate. Tool surface unchanged (zero golden diff; 134 tools): `restructure.gs`, `setup_apps_script.py`, the HMAC module, and the provisioning tools remain in place (now unused by this pipeline) and are removed in a follow-up strictly after the open OAuth review closes. Decision record: `_audit/2026-07-08-tabs-architecture-decision.md`; root-cause spike: `_audit/2026-07-08-exec-scope-auth-spike.md`.
...
- **/api/convert async job model - a client disconnect can no longer lose a conversion or strand its single-use URL (T1.1), plus batch + convert-from-Drive (T3.3) and full parameter documentation (T3.2)** (new `src/appscriptly/job_store.py`, `src/appscriptly/http_server/jobs.py`, `src/appscriptly/http_server/routes/convert_status.py`; reworked `routes/convert.py`, `middleware.py`, `crypto.py`; 2026-07 next wave). The field-feedback failure this closes: `/api/convert` was a synchronous multi-minute POST bound to the client connection; a client read-timeout mid-conversion silently lost the work AND burned the signed URL's nonce. Now the handler records a job row (SQLite `convert_jobs.db` on the `/data` volume: job_id, user_id, fingerprint, status queued|running|done|error, created_at/updated_at/heartbeat_at, result_json/error_json) and runs the converter in a DETACHED asyncio task (worker thread via `asyncio.to_thread` - this also stops the converter from blocking the event loop, which the old inline call did for minutes). The default sync response is unchanged (the handler awaits through an `asyncio.shield`, so cancelling the request coroutine leaves the job running - a bare await would cancel the job with its awaiter); `async=1` opts into an immediate `202 {job_id, status_url}` where `status_url` is pre-signed (same `signed_url` HMAC key family, domain-tagged canonical, 24h validity, deliberately multi-use/no-nonce) and `GET /api/convert/status/{job_id}` reports the status (+ full result when done; + the sync-equivalent http status when failed; tampered/expired signatures get 403). **Concurrency is capped application-side**: each runner acquires a per-loop semaphore (`CONVERT_JOB_MAX_CONCURRENCY`, default 2) BEFORE the row may claim `running`; jobs past the cap stay honestly queued (heartbeating while they wait) - without this, one batch could run ~5 genuinely concurrent multi-minute converts on the 512MB box (the #226 OOM class). **Nonce semantics:** the signed-URL nonce is consumed at JOB CREATION (moved out of the middleware for `POST /api/convert`), so a request rejected before work starts (validation error, missing creds, over-cap payload) no longer burns the URL; and a retry carrying the same request fingerprint (sha256 of user + file-content hash or Drive id + output-affecting params incl. `nest_by`/`on_conflict`) within 15 minutes ATTACHES to the existing job - returning its in-flight/completed/failed outcome (marked `attached_to_existing_job: true`) instead of duplicating work or documents, even on a burned nonce (`find_attachable_job` additionally predicates on `user_id` as defense in depth). **Deploy semantics (documented in the modules):** a Fly deploy/restart kills in-flight tasks; rows stop heartbeating (30s interval, best-effort writes that never kill the runner) and DERIVE `stalled` (heartbeat older than 120s) rather than lying `running` forever, and the identical re-POST re-arms the SAME job row (issued status URLs stay valid). **Batch (T3.3):** multiple multipart `file` parts or a `drive_file_ids` JSON array fan out to one job per input and always return `202 {jobs:[...]}`; single `drive_file_id` converts an existing app-accessible Drive file through the same pipeline (no upload); `markers` (the retrofit path: inject Heading 1s into a styled .docx, then convert) is now reachable over HTTP; the S2.5 partial-failure envelope (kept doc + manifest + `error`) maps to 500-with-body exactly like the sync contract. `gdrive_get_signed_upload_url`'s `usage_hint` now documents the FULL form-field surface (split_by, nest_by, title, icons_by_title, placeholder_behavior, markers, on_conflict, async, drive_file_id, drive_file_ids/batch) plus the retry/attach semantics. Tool surface unchanged (zero golden diff; HTTP layer only).
- **`gdrive_rename_file` - rename a Drive file in place** (`services/drive/api.py::rename_file`, wired in `services/drive/tools.py`; 2026-07 next wave, BUG 2b). `files.update` on the `name` field only - id, content, comments, sharing, and location untouched. Exists because the convert pipeline historically imported under a temp name and renamed LAST, so an interrupted run left a finished doc named "tmpjgehtmo2" with no tool to fix it. Same soft-failure contract as trash/untrash (`not_found` / `app_not_authorized` returned as data); same `drive.file` scope - additive, no new scope. Canonical `gdrive_` name only, NO deprecated alias (the tool never had a legacy name).
- **`server_health` - three-layer health report** (`services/admin/tools.py`; 2026-07 next wave, T1.2). One no-arg call reports `{server, google_api, google_api_detail, automation_runtime}`: a cheap credentialed `drive.about.get` classifies Google API access as ok / unauthorized / error; the per-user Apps Script runtime reports `installed` + `exec` in {`serving`, `needs_activation`, `api_disabled`, `not_installed`, `unknown`} with a `remediation_url` per state. Detects the account-level "Apps Script API disabled" case by its distinctive 403 from a cheap `script.projects.get` and points at https://script.google.com/home/usersettings; a DEAD `/exec` probe (Google's 403 door page) maps to `needs_activation` with the script-editor URL for the one-time interactive Allow. Never raises for auth problems and never triggers a consent flow - it reports (the stdio path peeks at the cached token non-interactively instead of calling the flow-launching loader). Reuses `setup_apps_script.probe_webapp_health`. Canonical-only, no alias.
...
- **Per-user write governor + rate-limit requeue: an 8-job storm completes 8/8 with zero manual steps (A2, retest 3)** (`services/docs/content_transplant.py`, `http_server/jobs.py`, `http_server/routes/convert.py`, `docx_import.py`). Round 3's storm test showed the N2 backoff working but bleeding: the 60-writes/min Docs quota is PER USER, and concurrent jobs competed each other into 429s until one job's ~75s retry budget exhausted and it died terminally (7/8). Two layers:
  - **Root - write governor:** every transplant/carve batchUpdate SEND first takes a per-user pacer slot (`DOCS_WRITE_MIN_INTERVAL_SECONDS`, default 1.1s for a 55/min target under the 60/min quota; one governor per user key, shared across the worker threads of all that user's concurrent jobs; reservation under a lock, sleep outside it). Governor waits are ordinary pacing - they consume NO 429 retry budget - so saturation stops happening at the source. `docx_import` keys the governor by the route's ``user_id`` (None = the operator bucket).
  - **Safety net - requeue:** a failure whose ROOT CAUSE is the write quota (HTTP 429 past the backoff budget - detected via the partial envelope's new ``rate_limited: true`` flag or the raised error's cause chain) no longer goes terminal: the runner re-arms the row (status honestly back to ``queued``, heartbeating through the whole wait, concurrency slot RELEASED so other jobs run), waits out the quota window (``CONVERT_JOB_REQUEUE_DELAY_SECONDS``, default 65s), and re-runs the work, up to ``CONVERT_JOB_MAX_REQUEUES`` (default 1) times. The retry receives the prior attempt's kept-doc envelope and trashes the superseded partial doc (recoverable 30 days) before rebuilding from the still-intact source, so automatic retries never accumulate litter. A job that exhausts its requeues fails terminally with the LAST envelope plus ``requeue_attempts``; every non-rate-limit failure keeps the N1/N3 terminal semantics byte-for-byte. Work closures now take the prior envelope as their argument (runner contract change, internal).
- **Retest round 3 semantics (N8/N9/A1, operator dogfood on build 370096c)** - tool surface unchanged (zero golden diff; 136 tools).
  - **N8 (MEDIUM-HIGH): explicit titles are honored VERBATIM on every entry point; the " (tabified)" suffix applies ONLY to the no-title gdoc-copy fallback** (`services/drive/api.py::copy_google_doc`, `docx_import.py::_expected_final_title` - the two are kept in lockstep). The drive_file_id path suffixed even an explicit title AND ran its on_conflict lookup against the suffixed name, so drive-path skip returned created while an upload-path doc with the requested title was live, and drive-path replace trashed only the suffixed doc. The on_conflict lookup now runs against the FINAL effective title with identical logic on upload, batch, and drive entries - pinned by a cross-entry-point (upload | drive_gdoc | drive_docx) x (skip | replace) test matrix whose load-bearing assertion is the lookup QUERY itself, plus verbatim-title unit pins on both helpers.
  - **N9 (LOW): the polling error envelope no longer double-nests** (`http_server/routes/convert_status.py::job_status_view`). ``status="error"`` payloads exposed the message at ``error.error``; the view now renames the inner key so the message lives at ``error.message`` alongside the recovery fields (doc_id, completion, warnings). The SYNC path's 500 body is untouched (the S2.5 envelope keeps its historical top-level ``error`` key there); usage_hint + poller docs updated. Shipped one day after #232, before consumers ossify.
  - **A1 (decision record, no behavior change): same-URL re-POST after a FAILED attempt stays 401** - single-use nonce semantics kept (refunding a consumed nonce on failure reopens replay ambiguity; fingerprint dedup already makes fresh-URL retries safe). Verified legible end to end: the burned-nonce 401 says "Mint a fresh signed upload URL and retry", the failed-job status note says the same, and usage_hint documents it - all three already test-pinned.

- **Retest cycle N1-N6 (operator dogfood on build 223b1f9, 2026-07-10)** - four new findings fixed, two doc/UX steers applied; tool surface unchanged (zero golden diff; 136 tools).
  - **N1 (HIGH): failed jobs no longer poison the 15-minute idempotency window** (`http_server/routes/convert.py::_plan_input`). A convert job that ended in failure was still an attach target, so every identical re-POST for 15 minutes replayed the cached failure - the error text's own "re-run the conversion" advice could not work without mutating a param to shift the fingerprint. Attach eligibility is now: queued/running (live task), stalled (re-arm, unchanged), and done WITHOUT an error field; a re-POST matching a FAILED job starts a NEW job (the burned-nonce 401 already instructs minting a fresh signed URL, which is the truthful recovery path). Rows persisted by a pre-N3 build as done-with-error-envelope are treated as failed too. The resume-from-``pending_sections`` idea stays open as a follow-up; this cycle deliberately ships the simple correct semantics.
  - **N3 (MEDIUM): a partial failure now polls as terminal ``status="error"``, so ``done`` MEANS success** (`http_server/jobs.py::_run_job`, `routes/convert_status.py`). A converter that RETURNS the S2.5 kept-doc recovery envelope (``error`` + completion manifest) previously finished the row as ``done`` with the failure buried in ``result.error`` - machine consumers read partial failures as wins (reviewer-flagged, field-confirmed). The runner now records those as ``status="error"`` with the FULL envelope as the error payload; the status endpoint serves it under ``error`` plus ``error_http_status`` (the sync path's 500-with-recovery-body stays byte-identical) and an honest retry note.
  - **N2 (MEDIUM): bounded 429 backoff in the transplant write path** (`services/docs/content_transplant.py::_batch_update`). Concurrent jobs tripping the per-user Docs write quota (WriteRequestsPerMinutePerUser=60) died on the FIRST 429 despite RATE_LIMIT_EXCEEDED being rejected-before-execution and therefore safe to retry even on writes. Every transplant/carve write now retries 429 (and ONLY 429 - a 5xx write may have partially executed and stays single-shot) with exponential backoff + full jitter against a shared ~75s budget per transplant entry; exhaustion re-raises into the existing keep-the-doc + completion-manifest failure path.
  - **N4 (MEDIUM, SECURITY): OAuth access tokens no longer reach the logs** (`oauth_google.py::_TokenInfoBodyClient`, `http_server/middleware.py::SensitiveQueryScrubFilter`). fastmcp's GoogleTokenVerifier validates every MCP call via ``GET tokeninfo?access_token=<live token>`` and httpx INFO-logs the full URL - replayable bearer tokens in the Fly log stream. Primary fix: a rewriting httpx client injected through GoogleProvider's public ``http_client`` param turns exactly that call into a POST with the token in the form body (Google accepts both; identical response), so the logged URL is token-free at the source. Defense in depth: every root log handler now carries a scrub filter that redacts credential-shaped query params (``access_token``/``token``/``sig``/``secret``/``key``/``nonce``/``assertion``/``password`` suffixes) from every record, whatever library emits it. Acceptance criterion held in tests: a captured log of the full verifier path contains no token substring.
  - **N5 (LOW): ``on_conflict=replace`` trashes EVERY app-visible same-title prior** (`docx_import.py`), not just the newest - iterating on a doc used to accumulate older duplicates that replace silently skipped. The response lists them newest-first in ``replaced_doc_ids`` (new, additive) with ``replaced_doc_id`` kept as the newest for existing callers; partial trash failures report exactly what happened.
  - **N6 (LOW): ``usage_hint`` steers large docs to ``async=1``** ("RECOMMENDED for docs with many sections; sync requests can take 60s+ and risk client timeouts") and documents the N1/N3/N5 semantics (done means success; failed attempts are not deduplicated; replace lists all trashed priors).
  - **R1 (retest 2): the from-Drive placeholder deviation diagnosed and made legible** (`docx_import.py`, tests across all three entry points). Mechanical reproduction shows the delete default IS applied identically by the upload, batch, and drive_file_id entries (now test-pinned at both the converter and HTTP layers, both Drive source kinds); the observed ``placeholder="kept"`` on a from-Drive convert is the S2.5 sole-copy guard vetoing the delete because the source doc carried visible content BEFORE its first Heading 1 (a title line - common in hand-authored Google Docs, rare in generated .docx, which is why only the drive path surfaced it). The veto is now machine-distinguishable from a keep POLICY: the response carries ``placeholder_veto: "unmoved_content"`` alongside the existing warning.
  - **R2 (retest 2): the first-tab delete advisory's tense now matches reality** (`services/docs/api.py::inspect_tab_content` gains ``original_first_tab_present`` - the same "t.0" convention the ``first_tab_deleted_500`` classifier keys on; `services/docs/tools.py::gdocs_delete_tab`). Deleting the original first tab says the defect is active NOW ("will now fail"); a document whose t.0 was already deleted gets "this document is ALREADY affected... this delete does not change that" instead of the misleading future tense, on both the refusal clause and the post-delete warning.
  - **R3 (retest 2): ``server_health`` states what the automation runtime gates** (`services/admin/tools.py`). Retest 2 proved 7/7 converts succeed while ``exec="needs_activation"`` (the convert path is pure REST since #222), yet the reading invited a false abort. Every ``automation_runtime`` payload now carries ``gates: "as_* automation tools only; convert (...) and all gdocs/gdrive/gsheets/gslides tools are unaffected"``, the needs_activation detail says conversions keep working, and the tool docstring spells out the blast radius.
- **`gdocs_read_doc` single-tab modes failed the tool's own output validation on every call (BUG 3 / S2.1)** (`services/docs/api.py::read_tab_content`). The declared `GDOCS_READ_DOC_OUTPUT_SCHEMA` requires `doc_id`, but the single-tab payload (both the `tab_id` and `tab_title` branches route through `read_tab_content`) never carried it - so FastMCP rejected the response with "'doc_id' is a required property" 4/4 deterministic in the field, blocking the verify-one-tab QA loop, while whole-doc mode passed. The payload now echoes `doc_id` (additive; schema untouched). Regression tests validate the ACTUAL payload of all three modes against the DECLARED schema with jsonschema (`tests/unit/services/docs/test_read_doc_schema.py`) - the check the old per-key tests never performed.
- **`gdocs_set_tab_icons` / `gdocs_rename_tab` deterministic Google 500 after a first-tab delete now returns a real diagnosis instead of "transient, retry" (S2.3)** (`services/docs/api.py::_classify_tab_props_500`). Live-reproduced root cause (controlled experiment on prod scratch docs, 2026-07-10): deleting a document's ORIGINAL FIRST TAB (the implicit `t.0`) triggers a Google-side defect - every subsequent `updateDocumentTabProperties` request on that document (icons AND renames, batch or single, even on tabs created later) fails with a durable 500; deleting a non-first tab does not, and the identical requests succeed before the delete, so the request shape (verified against the discovery doc) is not at fault. We cannot fix Google's backend; the fix reclassifies: when the batchUpdate 500s AND the freshly fetched root tab list carries no `t.0`, both tools raise a diagnosed, explicitly NON-retryable error naming the defect (`first_tab_deleted_500`) and the workarounds (set icons/titles BEFORE deleting the first tab; icons-at-creation still work). A 500 with `t.0` present keeps the generic retryable envelope. `gdocs_delete_tab` now also warns in its response when the tab it just deleted was the doc's first tab. NOTE for the convert pipeline: cosmetics (icons_by_title) must run BEFORE placeholder deletion or they will always hit this defect when `placeholder_behavior=delete`.
- **`gdocs_delete_tab` refuses to destroy content silently (S2.5 defense)** (`services/docs/tools.py`, new `services/docs/api.py::inspect_tab_content`). `deleteTab` is permanent (no trash, no undo) and the session-2 data-loss incident was a "placeholder" delete that destroyed the only copy of four unmoved sections. The tool now inspects the target tab FIRST: if the tab or any of its child tabs (cascade-deleted by the API) still holds meaningful content (non-whitespace paragraphs, tables, images), the call is refused with an error naming the guard (`non_empty_tab_guard`), the element counts, and the escape hatch - the new optional `force: bool = false` param. An unknown `tab_id` is now a clean pre-flight error instead of a blind Google 400.
- **Error envelope says whether retrying can help, and the transport absorbs one GET blip** (`errors.py`, `google_api_client.py`; next-wave polish). `friendly_http_error_message` gains a `Retryable: true|false` line (true only for Google's documented-transient 429/500/502/503/504) while keeping Google's own reason/details text verbatim - agents stop burning retries on 4xx caller bugs and on the S2.3 defect above. The Google transport (`_build_authorized_http`) is now wrapped in `_GetRetryHttp`: ONE bounded retry with backoff (honoring `Retry-After`, capped at 8s) for GET requests answered 429/5xx - the floor policy for the long tail of read paths that never adopted `execute_with_retry`; non-GET methods pass through byte-identical, so mutations keep their per-call-site idempotence contract.

- **`gdocs_add_tabs` failed output validation on EVERY call, after the tabs were already created (duplicate-tabs-on-retry trap)** (`src/appscriptly/services/docs/api.py::add_tabs_to_doc`). `GDOCS_ADD_TABS_OUTPUT_SCHEMA` requires `doc_id` + `url` + `tabs`, but the function returned only `{"tabs": ...}` (both the main path and the empty-input early return), so FastMCP rejected the response AFTER both mutating batchUpdates had landed; a client retrying the "failed" call duplicated the tabs (observed live in the 2026-07-02 demo). Now returns the same `{doc_id, url, tabs}` envelope as `make_doc_with_tabs` on both paths. Schema untouched (zero tool-surface diff); regression tests pin the schema-required keys on both return paths.
